'use client'
/**
 * Announcements Management - Client Component
 * Handles client-side interactivity and CRUD operations via API routes
 */
...
export function AnnouncementsClient({ initialAnnouncements = [] }: AnnouncementsClientProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedAnnouncement) {
        ...
        setAnnouncements(
          announcements.map((a) => (a.id === selectedAnnouncement.id ? data : a))
        )
      } else {
        // 추가
  ...
  const togglePublished = async (announcement: Announcement) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Failed to toggle published')
      }
