...
/**
 * Training Programs Management - Client Component
...
interface TrainingProgramsClientProps {
  initialPrograms: TrainingProgram[]
}

export function TrainingProgramsClient({ initialPrograms }: TrainingProgramsClientProps) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/training-programs/${selectedProgram.id}`, {
        method: 'DELETE',
      })
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
            ...
            {programs.map((program) => (
              ...
              >
                <Card sx={{ height: '100%' }}>
                    ...
                    <Stack spacing={1} sx={{ mt: 2 }}>
                      ...
                      {program.fee > 0 && (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="body2">참가비: {program.fee.toLocaleString()}원</Typography>
                        </Box>
                      )}
                    </Stack>
                  </CardContent>
                  ...
                  <CardActions>
                    <Button size="small" startIcon={<EditIcon />} onClick={() => handleEdit(program)}>
                      수정
                    </Button>
                    <Button
                      size="small"
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={4}>
                <TextField
                  ...
                  onChange={(e) =>
                    setEditingProgram({ ...editingProgram, duration_hours: Number(e.target.value) })
                  }
                  placeholder="2"
                />
              </Grid>
            ...
            <Grid container spacing={2}>
              <Grid size={6}>
                <TextField
                  ...
                  onChange={(e) =>
                    setEditingProgram({ ...editingProgram, max_participants: Number(e.target.value) })
                  }
                />
              </Grid>
              <Grid size={6}>
                <TextField
                  ...
                  onChange={(e) => setEditingProgram({ ...editingProgram, fee: Number(e.target.value) })}
                />
              </Grid>
            </Grid>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            {selectedProgram?.title} 프로그램을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
          </Typography>
        </DialogContent>
        <DialogActions>
